...
def init_csv():
    with open('data.csv', mode='w') as file:
        ...
        writer.writerow(['camera_name', 'area', 'shift', 'time', 'count'])
...
def update_entry_time(area, shift):
    ...
    with open('data.csv', mode='a') as file:
        writer = csv.writer(file)
        writer.writerow([camera_name, area, shift, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 1])
...
def update_exit_time(area, shift):
    ...
    with open('data.csv', mode='a') as file:
        writer = csv.writer(file)
        writer.writerow([camera_name, area, shift, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0])
...
class detect_tray:
    ...
    def process_camera(self):
        ...
        """
        video_stream = cv2.VideoCapture("rtsp://admin:admin123@10.0.22.205:554/cam/realmonitor?channel=1&subtype=0")
        frame_number = 0
